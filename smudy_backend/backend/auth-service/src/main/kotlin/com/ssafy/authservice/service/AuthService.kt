...
class AuthService(
    ...
    fun reissue(
        ...
        try {
            ...
            internalId = getPrincipal(accessToken)
        } catch (e: ExpiredJwtException) {
            val claims = e.claims
            authentication = jwtTokenProvider.getAuthenticationWithClaims(claims)
            internalId = claims["internal_id"].toString()
        }
        ...
        try {
            ...
            if (refreshTokenInRedis != requestRefreshToken) {
                // 리프래시 토큰이 탈취당했을 수 있음
